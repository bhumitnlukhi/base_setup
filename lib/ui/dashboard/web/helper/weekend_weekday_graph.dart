...
class WeekendWeekdayGraph extends ConsumerStatefulWidget {
  const WeekendWeekdayGraph({Key? key}) : super(key: key);

  @override
  ConsumerState<WeekendWeekdayGraph> createState() => _WeekendWeekdayGraphState();
}

class _WeekendWeekdayGraphState extends ConsumerState<WeekendWeekdayGraph> with BaseStatefulWidget, TickerProviderStateMixin {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      barGraphAnimationControllerList = List.generate(dashBoardWatch.monthGraphDataListDummy.length, (index) => AnimationController(vsync: this, duration: const Duration(milliseconds: 150)));
          ...
          child: Container(
            ...
            child: BarChart(
              BarChartData(
                  ...
                  titlesData: FlTitlesData(
                    ...
                    leftTitles: AxisTitles(
                      sideTitles: SideTitles(
                        showTitles: true,
                        reservedSize: 40,
                        interval: /*dashboardWatch.dashboardSalesReportState.isLoading ? 100 : */((dashboardWatch.monthGraphDataListDummy.map((e) => e).toList().max + 100)/4).round().toDouble(),
                  ...
                  maxY: /*dashboardWatch.dashboardSalesReportState.isLoading ? 16000 : */ dashboardWatch.monthGraphDataListDummy.map((e) => e).toList().max + 100,
  ...
  List<BarChartGroupData> getDataStatic() {
    final dashBoardWatch = ref.watch(dashboardController);
    const weekDayGradient = LinearGradient(
      begin: Alignment.topCenter,
      end: Alignment.bottomCenter,
      colors: [Color(0XFFFF7777), Colors.transparent],
    );
    const weekendGradient = LinearGradient(
      begin: Alignment.topCenter,
      end: Alignment.bottomCenter,
      colors: [Color(0XFFFFD700), Colors.transparent],
    );

    return List.generate(dashBoardWatch.monthGraphDataListDummy.length, (index) {
      final customAnimationWatch = ref.watch(customAnimationController);
      ...
      // Weekday = 0 to 4, Weekend = 5 and 6
      ...
      return BarChartGroupData(
        ...
        barRods: [
          BarChartRodData(
            toY: dashBoardWatch.monthGraphDataListDummy[index] * barGraphAnimationControllerList[index].value,
            width: 5.w,
            gradient: isWeekend ? weekendGradient : weekDayGradient,
  ...
  Widget bottomTitles(double value, TitleMeta meta) {
    ...
    return SideTitleWidget(
      ...
      space: 10,
      child: Text(dashBoardWatch.xTitlesDummy[value.toInt()], style: style),
    );
  }
