...
final calendarController = ChangeNotifierProvider((ref) => CalendarController());
...
class CalendarController extends ChangeNotifier {
  ...
  initAnimationController(customDatePickerState) {
    animationController = AnimationController(vsync: customDatePickerState, duration: const Duration(milliseconds: 150));
  ...
  disposeController(BuildContext context, customDatePickerState, {DateTime? initialDate, required DateTime firstDate, required DateTime lastDate}) {
    widgetDisplayed = WidgetDisplayed.day;
    if (firstDate != DateTime(1900) && lastDate != DateTime(DateTime.now().year + 50)) {
      if (lastDate.difference(firstDate).isNegative) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select proper dates')));
        Navigator.pop(context);
        return;
      }
      if (!(((firstDate.difference(initialDate ?? DateTime.now())).isNegative) && ((lastDate.difference(initialDate ?? DateTime.now())).isNegative))) {
    ...
    if (lastDate != DateTime(DateTime.now().year + 50) && firstDate == DateTime(1900)) {
      if (((lastDate.difference(initialDate ?? DateTime.now())).isNegative)) {
        selectedDate = lastDate;
        displayDate = lastDate;
      } else {
    ...
    selectedYear = numOfYears.where((element) => element.year == selectedDate.year).first;
    selectedMonth = month.where((element) => element.month == selectedDate.month).first;
    updateDayList();
    initAnimationController(customDatePickerState);
    notifyListeners();
  }
  ...
  goToPreviousMonth() {
    if ((animationController?.isCompleted ?? false) && isPreviousButtonVisible()) {
      ///Slide left to right
      slideAnimation = Tween<Offset>(begin: const Offset(-0.1, 0), end: Offset.zero).animate(animationController!);
  ...
  ///Go to next month
  goToNextMonth() {
    if ((animationController?.isCompleted ?? false) && isForwardButtonVisible()) {
      ///Slide right to left
      slideAnimation = Tween<Offset>(begin: const Offset(0.1, 0), end: Offset.zero).animate(animationController!);
      animationController?.reset();
  ...
  isPreviousButtonVisible() {
    if (firstDate != DateTime(1900)) {
      return !((displayDate.month - firstDate.month) == 0 && (displayDate.year - firstDate.year) == 0);
    }
    return true;
  }
  ...
  isForwardButtonVisible() {
    if (lastDate != DateTime(DateTime.now().year + 50)) {
      return !((displayDate.month - lastDate.month) == 0 && (displayDate.year - lastDate.year) == 0);
    }
    return true;
  }
  ...
  isDateAvailable(DateTime? dateTime) {
    if (firstDate == DateTime(1900) && lastDate == DateTime(DateTime.now().year + 50)) {
      return true;
    } else if (firstDate != DateTime(1900) && lastDate == DateTime(DateTime.now().year + 50)) {
      return ((dateTime?.year == firstDate.year && dateTime?.month == firstDate.month) ? ((dateTime?.day ?? DateTime.now().day) >= firstDate.day) : true);
    } else if (firstDate == DateTime(1900) && lastDate != DateTime(DateTime.now().year + 50)) {
      return ((dateTime?.year == lastDate.year && dateTime?.month == lastDate.month) ? ((dateTime?.day ?? DateTime.now().day) <= lastDate.day) : true);
    } else if (firstDate != DateTime(1900) && lastDate != DateTime(DateTime.now().year + 50)) {
      return ((dateTime?.year == firstDate.year && dateTime?.month == firstDate.month) ? ((dateTime?.day ?? DateTime.now().day) >= firstDate.day) : true) &&
          ((dateTime?.year == lastDate.year && dateTime?.month == lastDate.month) ? ((dateTime?.day ?? DateTime.now().day) <= lastDate.day) : true);
    }
    return true;
  }
  ...
  isMonthAvailable(index) {
    ...
    if (firstDate == DateTime(1900) && lastDate == DateTime(DateTime.now().year + 50)) {
      ...
      return (dateTime.year > firstDate.year || (dateTime.year == firstDate.year && dateTime.month >= firstDate.month));
    } else if (firstDate == DateTime(1900) && lastDate != DateTime(DateTime.now().year + 50)) {
      return (dateTime.year < lastDate.year || (dateTime.year == lastDate.year && dateTime.month <= lastDate.month));
    } else if (firstDate != DateTime(1900) && lastDate != DateTime(DateTime.now().year + 50)) {
      return ((dateTime.year > firstDate.year || (dateTime.year == firstDate.year && dateTime.month >= firstDate.month)) &&
          (dateTime.year < lastDate.year || (dateTime.year == lastDate.year && dateTime.month <= lastDate.month)));
    }
  ...
  updateDayList({int? month, int? year}) {
    dayList = daysList(month ?? displayDate.month, year ?? displayDate.year) ?? [];
  ...
  ///Default Animation
  Animation<Offset> get defaultAnimation => Tween<Offset>(begin: const Offset(1, 0), end: Offset.zero).animate(animationController!);
  ...
  List<DateTime>? daysList(final int monthNum, final int year) {
    ...
    for (int day = 0; day < numOfDays; day++) {
      dayList.add(DateTime(year, monthNum, (day + 1), DateTime.now().hour, DateTime.now().minute, DateTime.now().second));
    }
    return dayList;
  }
  ...
  List<List<DateTime?>>? getDayWiseMonthlyList(List<DateTime> dayList) {
    ...
    for (dayIndex = 0; dayIndex < dayWiseMonthlyList.length; dayIndex++) {
      List<DateTime?> element = dayWiseMonthlyList[dayIndex];
      Duration? diff = dayWiseMonthlyList[dayIndex].first?.difference(dayWiseMonthlyList.last.first ?? DateTime.now());
